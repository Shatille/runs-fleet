...
func (w *WarmPoolAssigner) TryAssignToWarmPool(ctx context.Context, job *queue.JobMessage) (*WarmPoolResult, error) {
	...
	warmPoolLog.InfoContext(ctx, "instance claimed")
	...
	if err := w.Runner.PrepareRunner(ctx, prepareReq); err != nil {
		warmPoolLog.ErrorContext(ctx, "runner config preparation failed",
		...
		for attempt := 0; attempt < 3; attempt++ {
			...
			if stopErr := w.Pool.StopPoolInstance(ctx, instance.InstanceID); stopErr != nil {
				warmPoolLog.ErrorContext(ctx, "instance stop failed after SSM prep failure",
		...
		if !stopped {
			warmPoolLog.ErrorContext(ctx, "instance may have invalid config - manual cleanup required")
		}
		...
		if w.DB != nil {
			if releaseErr := w.DB.ReleaseInstanceClaim(ctx, instance.InstanceID, job.JobID); releaseErr != nil {
				warmPoolLog.ErrorContext(ctx, "instance claim release failed",
	...
	if w.DB != nil && w.DB.HasJobsTable() {
		...
		if err := w.DB.SaveJob(ctx, jobRecord); err != nil {
			warmPoolLog.ErrorContext(ctx, "job record save failed",
				slog.String("error", err.Error()))
			...
			for attempt := 0; attempt < 3; attempt++ {
				...
				if stopErr := w.Pool.StopPoolInstance(ctx, instance.InstanceID); stopErr != nil {
					warmPoolLog.ErrorContext(ctx, "instance stop failed after DB save failure",
						slog.Int("attempt", attempt+1),
						slog.String("error", stopErr.Error()))
					continue
				}
			...
			if !stopped {
				warmPoolLog.ErrorContext(ctx, "instance may be running without job record - manual cleanup required")
			}
			return &WarmPoolResult{Assigned: false}, nil
		}
	}

	warmPoolLog.InfoContext(ctx, "job assigned to warm pool")
