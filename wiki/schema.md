...
- `github-integration`: `pkg/github` — GitHub App API client (auth, registration tokens, workflow-job status; relocated from `pkg/runner` in PR #380) + webhook HMAC-SHA256 validation + label/alias parsing. JIT configs ARE minted here (`pkg/github/jitconfig.go`, PR #424) and are preferred over registration tokens — but a JIT config makes a runner *ephemeral*, not job-bound: dispatch stays label-matched. PR #430 corrected that comment in `jitconfig.go` only; `pkg/runner/manager.go`, `pkg/secrets/store.go`, and `pkg/agent/executor.go` still assert the false job-bound claim.
...
- `housekeeping`: `pkg/housekeeping` — orphan sweeps, stale/hung-job requeue, orphaned GitHub runner deregistration, stale-AMI retirement, fleet cost sampling, TTL'd entities. Scheduling is in-process timers plus a DynamoDB task lock as of 2026-08 (`runner.go`); the SQS scheduler/handler pair and the pool-audit task were both removed.
- `admin-ui`: `pkg/admin` REST API + embedded Next.js UI; native OIDC auth (authorization-code + PKCE, HMAC-signed session cookie) as of 2026-07 — no external gatekeeper required. Reads DynamoDB/EC2/SQS/S3 only; no CloudWatch metric reads anywhere in the package.
...
- `infrastructure`: Dockerfiles, Packer (two-layer base + runner AMIs, **dual-arch** arm64 and amd64), Helm chart (K8s only), Nix flake, Makefile, GitHub Actions workflows, Trivy CVE gate.
- `registry-mirroring`: `pkg/mirrorproxy` + `cmd/mirror-proxy` — the host-local Docker Hub mirror that forwards onto an ECR pull-through cache, plus the Packer/systemd wiring and the dockerd/BuildKit config it generates. Added 2026-08-21.
- `build-caching`: `pkg/buildxshim` + `cmd/buildx-shim` — the buildx CLI-plugin shim that transparently injects S3-backed layer cache (`type=s3`) and a `--buildkitd-config` attach, requiring no workflow changes. Added 2026-08-21.
...
- `absent-is-not-zero`: A quantity that could not be observed is carried as a distinct state (second return value, omitted field, `nil` result, explicit "unavailable") rather than collapsing into the zero value — because a measured zero and an unmeasured quantity render identically. PR #403's fabricated "1 job / 0.0 hours" is the failure that motivates it. The *control-flow* variant (added 2026-09-15) is the same collapse applied to outcomes rather than data: #457's sweep returning `nil` after logging every failure, #458's fallback latch muting its own warning — connects [observability, admin-ui, state-storage, housekeeping].
...
- 2026-08-21: Incremental compile on the fix/cloudwatch-metrics-default-off branch (~100 commits since 2026-07-21). `housekeeping` and `admin-ui` rewritten against current source. Two structural corrections: the housekeeping scheduler is now in-process timers + a distributed task lock (`pkg/housekeeping/runner.go`) with `scheduler.go`/`handler.go` deleted and nothing publishing to or consuming from the housekeeping SQS queue; and `TaskPoolAudit`/`ExecutePoolAudit` were removed entirely (a second, staler writer into the same PoolInstances/PoolDesired series pool reconciliation already writes, paid for with a non-paginated pools-table Scan every 10 minutes). New housekeeping tasks documented: `orphaned_runners` (#436/#437), `stale_ami_instances` (#431-#434), stale-queued auto-requeue (#430) and the `requeued` orphan case (#423), `unconfirmed_runners` (#418), `pool_hot_tuner` (#399/#401), `expired_instance_claims`, `fleet_cost_sample` (#455). New admin handlers documented: `handler_ami.go`, `handler_replace_stale.go`, `handler_hung.go`, `handler_requeue.go` (per-job requeue/reconcile), `handler_job_logs.go`, the on-demand orphaned-instance reaper, and the optional Fleet Cost block. Corrected a stale `admin-ui` gotcha: `requeue-hung-jobs` now persists through `recordAdminAction`, so it does appear in `GET /api/audit-logs`. Recorded the two facts that matter across topics: `ExecuteOldJobs` hard-deletes job rows at 7 days with no archive (so any jobs-table-derived MTD figure is truncated), and the admin console makes no CloudWatch metric-read calls (so shipping the CloudWatch backend disabled by default does not affect it).
- 2026-08-21: Incremental compile on the fix/cloudwatch-metrics-default-off branch (~100 commits since 2026-07-21). New topic `registry-mirroring` for the PR #439-#453 mirror-proxy series (no prior article covered `pkg/mirrorproxy` or `cmd/mirror-proxy`). `observability` rewritten: PR #455 fleet-level cost attribution (`pkg/cost/fleetpricing.go`, `fleetmtd.go`, `pkg/db/fleet_cost.go` `__fleet_day:` rollups, `pkg/housekeeping/fleet_cost.go` 60s sampler, `RUNS_FLEET_REPORT_TIMEZONE` via `pkg/config/timezone.go`), the CloudWatch backend now shipping disabled (PR #456: default flipped in both `pkg/config/config.go` and the Helm values, since nothing queries the RunsFleet namespace), CloudWatch put-rate/rollup/statistic gotchas recorded, and two stale claims corrected -- the 10k `jobWindowLimit` cap and `Breakdown.JobsMatched` were removed by PR #403 (2026-08-05) and are now marked obsolete rather than documented as current. Also noted PR #446 dropping the never-permitted CloudWatch Logs paths and PR #445 adding S3 runner-log shipping (detail belongs to agent-runtime/admin-ui). Untouched this pass and still due: the other 17 topics and all 4 concepts.
- 2026-09-15: Incremental compile on the `fix/runner-dotnet-install-dir` branch (4 commits since 2026-08-21: #457, #458, #459, and the unmerged c4c3b01). 7 topics updated — `infrastructure` and `cmd-agent` for the .NET change, `observability`/`housekeeping`/`cmd-server`/`project-overview` for #458's cost-pricing rebuild, `housekeeping`/`state-storage` for #457's GSI-key defect, `infrastructure`/`cmd-agent`/`observability` for #459. Concept `absent-is-not-zero` gained two *inverse* instances and a new synthesis paragraph naming the **control-flow zero**: `#457`'s sweep logged every per-record failure and still `return nil`, and `#458`'s price-fallback latch suppressed its own warning — in both, the mechanism that degraded gracefully was also the mechanism deciding whether to report, so "wholly broken" and "healthy" emitted identical signals. One structural fact added to the schema's mental model: `setup-dotnet` consults **no** tool cache, so the `/opt/hostedtoolcache` prebake pattern that `infrastructure` documents for Python/Ruby/Go/Node/Java is *not* general — it is per-action, and .NET is the first documented exception. Four source-side doc drifts surfaced and deliberately left unfixed (recorded in INDEX Recent Changes): stale three-family fallback claims in `pkg/housekeeping/tasks.go` and `AGENTS.md:354-355`, a `CloudWatch Agent: enabled` echo at `packer/provision-base.sh:876`, and a "us-east-1" comment at `pkg/cost/fleetpricing.go:11`. Previous compiles' line-number cites had drifted in `housekeeping` (3 cites) and `observability` (6 cites); corrected against current source.
