...
**externally-defined custom label** — e.g. an Actions Runner Controller (ARC)
...
Actions runners* (such as ARC) onto runs-fleet. Teams keep their current
`runs-on:` labels verbatim; runs-fleet learns to recognize and serve them, so
the shared/self-hosted runners can be sunset without touching any workflow
manifest.
...
- Operators supply a JSON array of alias rules via `RUNS_FLEET_LABEL_ALIASES`,
  ...
  uncompilable regex or unparsable literal spec fails the boot). The shared
  label vocabulary is org-specific, so it lives in config, **never in source** —
  an empty/unset value reproduces pre-alias behavior exactly.
...
- The matched custom label is preserved as `OriginalLabel`, so the booted
  ...
  `runs-on: shared-8cpu-arm64` and `runs-on: [self-hosted, shared-8cpu-arm64]`
  match, because GitHub auto-adds `self-hosted`/`Linux`/`ARM64`.

**Caveat — competition during migration.** While both fleets are live, the
original Actions runners (e.g. ARC) and runs-fleet **compete** for the same
queued jobs; whichever registers/claims a runner first wins, and the loser's
ephemeral runner self-terminates.
...
Example rule covering an entire `shared-<N>cpu-<arch>` family (and any future
size) with one regex, pinning vCPU exactly for parity:
