...
5. Initialize core deps: `initJobQueue` (SQS), `db.NewClient` (plus optional GSIs via `SetJobsPoolStatusGSI` / `SetJobsInstanceIDGSI`, and the audit table via `SetAuditTable` when `cfg.AuditTableName` is set), `cache.NewServer`, `initMetrics` (installs the publisher into the AWS observability recorder).
...
- Admin handlers under `/api/` (rate-limited, gated by `admin.NewAuthMiddleware(cfg.AdminSessionSecret)` — native OIDC session cookies as of 2026-07, not Keycloak header trust): `admin.NewAuthHandler` (`/api/auth/{login,callback,logout,config}`), `admin.NewHandler` (takes `ws.dbClient` twice — once as the data client, once as the audit logger backing `GET /api/audit-logs`), `admin.NewJobsHandler`, `admin.NewInstancesHandler`, `admin.NewQueuesHandler` (configured with main / DLQ / pool / events / termination / housekeeping queue URLs), `admin.NewCircuitHandler`, `admin.NewHousekeepingHandler` (also takes `ws.dbClient` for audit persistence), `admin.NewRequeueHandler`, `admin.NewCostHandler`, and `admin.NewMetricsHandler` (Phase 1 read dashboards, PR #384 — takes the cost handler as its `costMTDProvider` so the metrics summary and cost page report the same month-to-date figure).
...
- `POST /webhook` — `handleWebhook`: validates HMAC via `gh.ParseWebhook(r, cfg.GitHubWebhookSecret)`, then `processWebhookEvent` performs durable work synchronously and returns a `postAck` closure for best-effort work run after the response is written (`runPostAck`, on a background context, panic-recovered):
  ...
  - `action=in_progress` → `handler.HandleWorkflowJobInProgress` (pure filter, no I/O) derives a `StartupObservation` from GitHub's own `created_at`/`started_at` timestamps, only when the runner name carries the `runs-fleet-` prefix AND the labels parse via `ParseLabelsWithAliases`; postAck calls `handler.PublishJobStartupMetrics`, which resolves the source (`warm_pool`/`cold_start` from `events.JobInfo.WarmPoolHit`) with one `GetJobByJobID` read and publishes `JobStartupSeconds` (PR #387). `ws.dbClient` (a concrete `*db.Client`) is guarded against the typed-nil trap before being assigned to the `handler.JobStartupDB` interface.
...
- **2026-07: `in_progress` webhook drives the startup-latency metric (PR #387).** The `JobStartupSeconds` distribution is computed from GitHub's own `created_at`/`started_at` timestamps, not orchestrator clocks — both timestamps come from the same GitHub clock, so cross-host skew cannot corrupt the span. Because `in_progress` fires for *every* runner in a repo (including foreign ones), the handler requires two-sided proof of ownership — the `runs-fleet-` runner-name prefix AND a successful label parse — the same filter pair `HandleJobFailure` uses. The durable webhook path stays cheap: the filter is pure (no I/O), and the single `GetJobByJobID` read plus the metric publish run post-ack.
