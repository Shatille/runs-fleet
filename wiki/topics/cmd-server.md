...
- Admin handlers under `/api/` (rate-limited, gated by `admin.NewAuthMiddleware(cfg.AdminSessionSecret)` — native OIDC session cookies as of 2026-07, not Keycloak header trust): `admin.NewAuthHandler` (`/api/auth/{login,callback,logout,config}`), `admin.NewHandler`, `admin.NewJobsHandler`, `admin.NewInstancesHandler`, `admin.NewQueuesHandler` (configured with main / DLQ / pool / events / termination / housekeeping queue URLs), `admin.NewCircuitHandler`, `admin.NewHousekeepingHandler`, `admin.NewRequeueHandler`, `admin.NewCostHandler`.
