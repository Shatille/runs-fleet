...
**Topics updated:** infrastructure, cmd-agent, observability, housekeeping, cmd-server, state-storage, project-overview (7)
**New topics:** none
**Concepts:** absent-is-not-zero (2 new inverse instances + "control-flow zero" synthesis)
**Sources scanned:** ~187 (15 changed files across 4 commits, plus full source re-reads for the 7 recompiled topics)
**Sources changed:** 15 files since 2026-08-21 (#457, #458, #459, c4c3b01)
**Notes:** Compiled on the fix/runner-dotnet-install-dir branch so the wiki lands with the .NET fix it documents. Headline structural finding: `actions/setup-dotnet` performs **no tool-cache lookup**, so the `/opt/hostedtoolcache` prebake pattern documented for setup-python/setup-ruby/setup-go/setup-node does not generalise — `DOTNET_INSTALL_DIR` pointed at an ec2-user-writable `/opt/dotnet` is the only lever, nothing is prebaked, and `ToolCacheMisses` is blind to .NET because `/opt/dotnet` sits outside `AGENT_TOOLSDIRECTORY`. Four pre-existing source-side doc drifts surfaced and left unfixed (stale three-family fallback claims in `pkg/housekeeping/tasks.go` and `AGENTS.md:354-355`; `CloudWatch Agent: enabled` echo at `packer/provision-base.sh:876` after #459; "us-east-1" comment at `pkg/cost/fleetpricing.go:11`) — all comments/output only. Nine drifted line-number cites from earlier compiles corrected (3 in housekeeping, 6 in observability). One mis-tagged heading fixed in cmd-server (`API Surface` was `high -- 2`, now `medium`).
