		...
		case <-ticker.C:
			...
			if err != nil {
				if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
					h.logger().WarnContext(ctx, "receive messages failed", slog.String("error", err.Error()))
				} else {
					h.logger().ErrorContext(ctx, "receive messages failed", slog.String("error", err.Error()))
				}
				continue
			}
			...
			for _, msg := range messages {
				if err := h.processMessage(ctx, msg); err != nil {
					h.logger().ErrorContext(ctx, "process message failed", slog.String("error", err.Error()))
				}
			}
		}
	}
...
func (h *Handler) processMessage(ctx context.Context, msg queue.Message) error {
	...
	if h.taskLocker != nil {
		...
		defer func() {
			...
			if err := h.taskLocker.ReleaseTaskLock(releaseCtx, string(hkMsg.TaskType), h.instanceID); err != nil {
				h.logger().ErrorContext(releaseCtx, "task lock release failed",
	...
	default:
		h.logger().WarnContext(taskCtx, "unknown task type")
	}

	if err != nil {
		h.logger().ErrorContext(taskCtx, "task failed",
			slog.String("error", err.Error()))
		return err
	}
