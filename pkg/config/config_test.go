...
func TestLoadBuildkitCacheBucket(t *testing.T) {
	originalEnv := os.Environ()
	t.Cleanup(func() {
		os.Clearenv()
		for _, e := range originalEnv {
	...
	t.Run("unset means disabled", func(t *testing.T) {
		...
		if cfg.BuildkitCacheBucket != "" {
			t.Errorf("BuildkitCacheBucket = %q, want empty", cfg.BuildkitCacheBucket)
		}
	})

	t.Run("region defaults from AWS_REGION", func(t *testing.T) {
		os.Clearenv()
		for k, v := range baseEnv {
			_ = os.Setenv(k, v)
		}
		_ = os.Setenv("AWS_REGION", "eu-west-1")
		_ = os.Setenv("RUNS_FLEET_BUILDKIT_CACHE_BUCKET", "my-cache-bucket")
		cfg, err := Load()
		if err != nil {
			t.Fatalf("Load() error = %v", err)
		}
		if cfg.BuildkitCacheBucket != "my-cache-bucket" {
			t.Errorf("BuildkitCacheBucket = %q, want my-cache-bucket", cfg.BuildkitCacheBucket)
		}
		if cfg.BuildkitCacheRegion != "eu-west-1" {
			t.Errorf("BuildkitCacheRegion = %q, want eu-west-1 (AWS_REGION default)", cfg.BuildkitCacheRegion)
		}
	})

	t.Run("explicit region overrides AWS_REGION", func(t *testing.T) {
		os.Clearenv()
		for k, v := range baseEnv {
			_ = os.Setenv(k, v)
		}
		_ = os.Setenv("AWS_REGION", "eu-west-1")
		_ = os.Setenv("RUNS_FLEET_BUILDKIT_CACHE_BUCKET", "my-cache-bucket")
		_ = os.Setenv("RUNS_FLEET_BUILDKIT_CACHE_REGION", "us-east-2")
		cfg, err := Load()
		if err != nil {
			t.Fatalf("Load() error = %v", err)
		}
		if cfg.BuildkitCacheRegion != "us-east-2" {
			t.Errorf("BuildkitCacheRegion = %q, want us-east-2", cfg.BuildkitCacheRegion)
		}
	})
}
