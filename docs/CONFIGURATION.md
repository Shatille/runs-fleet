...
| `RUNS_FLEET_POOLS_TABLE` | | Pool configurations (also holds per-pool reconciliation locks and per-instance warm-pool claim rows). Expired `__instance_claim:` rows are swept by the `expired_instance_claims` housekeeping task (every 15 min) and, as a storage-level backstop, by DynamoDB TTL on `claim_expiry`; neither touches pool-config or lock rows. See `deploy/terraform/dynamodb.tf` |
...
| `RUNS_FLEET_STALE_AMI_SWEEP_ENABLED` | `false` | Hourly sweep that retires **stopped** pool instances not running their architecture's current launch-template AMI, so the pool relaunches them on it. EC2 does not re-image on start, so a stopped member keeps its creation-time AMI forever and nothing else replaces it. At most one instance per pool per cycle, and never one whose reference AMI could not be freshly confirmed. Off by default because it terminates instances on a timer |
