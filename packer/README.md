...
- **A workflow that brings its own buildkitd config is on its own.** The shim skips those (`skipped:user-config`), so the config must name the bridge gateway itself. `devsisters/docker-setup-buildx-action` is one of these; it pointed BuildKit at the host's primary-ENI IP, which the proxy does not bind, so every build using it fell through to Docker Hub. It now probes for an address that answers, preferring the gateway.
