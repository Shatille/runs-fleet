...
# changes are part of the snapshot. Runs under `bash -euo pipefail` so a failure
# mid-hook aborts the build instead of being masked by the exit status of the
# hook's last command.
HOOK="/tmp/provision-base-hook.sh"
if [ -s "$HOOK" ]; then
  echo "==> Running downstream provision hook"
  sudo bash -euo pipefail "$HOOK"
fi
